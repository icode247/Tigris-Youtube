...
async function handleGet(
  ...
  try {
    ...
    const item = await videosCollection.findOne({ id: itemId });
    if (!item) {
...
async function handlePut(req: NextApiRequest, res: NextApiResponse<Data>) {
  try {
    ...
    const videosCollection = tigrisDB.getCollection<VideoModel>(VideoModel);
    const updated = await videosCollection.insertOrReplaceOne(item);
...
async function handleDelete(
  ...
  try {
    const videosCollection = tigrisDB.getCollection<VideoModel>(VideoModel);
    const status = (await videosCollection.deleteOne({ id: itemId })).status;
    if (status === "deleted") {
      res.status(200).json({});
    } else {
