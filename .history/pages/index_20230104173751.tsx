...
const Home: NextPage<Props> = ({ videos }) => {
  ...
  */
  const searchQuery = (searchInput: string) => {
    ...
    fetch(`/api/items/search?q=${encodeURI(searchInput)}`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  */
  ...
  return (
    <div>
      ...
      <div>
        <PrimarySearchAppBar createHandler={createHandler}  searchQuery={searchQuery}/>
        <Container maxWidth="lg" sx={{ mt: 4 }}>searchQuery
